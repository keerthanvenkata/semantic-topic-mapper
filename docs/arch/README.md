# Architecture (arch)

Detailed architecture documents linked from [System Architecture](../system_architecture.md).

| Document | Contents |
|---------|----------|
| [Two-Layer Architecture](two_layer_architecture.md) | Deterministic backbone vs LLM layer, principles, responsibility table, compiler analogy |
| [Topic Modeling Foundations](topic_modeling.md) | TopicID, TopicBlock, TopicNode, Subclause, topic ID grammar, header detection (patterns + title/standalone-ID heuristics), list markers |
| [References and Subclauses](references_and_subclauses.md) | Subclauses ≠ topics; TopicReference; graph edges connect TopicNodes only |
