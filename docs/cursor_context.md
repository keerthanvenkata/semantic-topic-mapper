...
**Status:** Phase complete. Semantic enrichment uses **Google Gemini** (e.g. **gemini-3-flash-preview**) via **google.genai**. The LLM client and entity enricher (types, relationships, ambiguity) are implemented; config: `LLM_API_KEY`, `LLM_MODEL`, `SKIP_LLM`, `LLM_DEBUG`.
